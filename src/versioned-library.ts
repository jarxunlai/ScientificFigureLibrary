...
export const CAPTURE_MATERIALIZATION_RECEIPT_SCHEMA =
...
export interface CaptureRevisionBinding {
  captureId: string;
...
export interface VersionedTemplateCandidate {
  ...
  captureBinding?: CaptureRevisionBinding;
...
export interface CaptureMaterializationReceiptV1 {
  schema: typeof CAPTURE_MATERIALIZATION_RECEIPT_SCHEMA;
  receiptId: string;
  captureId: string;
  templateId: string;
  revisionId: string;
...
export interface LifecycleOperationIntentV1 {
  ...
  captureReceipt?: CaptureMaterializationReceiptV1;
...
async function prepareCandidate(options: {
  ...
  for (const input of candidate.assets) {
    if (!["visual", "code", "data", "metadata", "context", "provenance"].includes(input.role)) {
    ...
    storedAssets.push({
      ...
      sha256: sha256(bytes),
      ...(input.origin !== undefined ? { origin: jsonClone(input.origin) } : {}),
    });
    sources.push({ ...source, logicalPath });
  }
  storedAssets.sort((left, right) => left.logicalPath.localeCompare(right.logicalPath));
  ...
  if (figureCodeLinks.some((link) => !link.evidence || !link.codeAssetPaths.length)) {
    throw new Error("figure-code links require evidence and at least one code asset");
  }
  if (
  ...
  if (visualGrouping && visualGrouping.confirmedBy !== "user") {
    throw new Error("multi-image grouping must be explicitly confirmed by the user");
  }

  const captureBinding = candidate.captureBinding
    ? {
        captureId: assertSafeSegment(candidate.captureBinding.captureId, "captureId"),
        requiredAssetSha256: uniqueStrings(
          candidate.captureBinding.requiredAssetSha256,
          "captureBinding.requiredAssetSha256",
        ).sort(),
        ...(candidate.captureBinding.selectionDigest
          ? { selectionDigest: candidate.captureBinding.selectionDigest }
          : {}),
      }
    : undefined;
  if (captureBinding) {
    if (!captureBinding.requiredAssetSha256.length) {
      throw new Error("capture binding requires at least one materialized asset digest");
    }
    if (captureBinding.requiredAssetSha256.some((digest) => !HASH.test(digest))) {
      throw new Error("capture binding contains an invalid SHA-256 digest");
    }
    const inventory = new Set(storedAssets.map((asset) => asset.sha256));
    if (captureBinding.requiredAssetSha256.some((digest) => !inventory.has(digest))) {
      throw new Error("capture binding is not self-contained in the candidate asset inventory");
    }
    if (captureBinding.selectionDigest && !HASH.test(captureBinding.selectionDigest)) {
      throw new Error("capture selectionDigest must be a SHA-256 digest");
    }
    if (!isRecord(candidate.provenance) || candidate.provenance.captureId !== captureBinding.captureId) {
      throw new Error("capture binding requires matching self-contained provenance");
    }
  }
  ...
  const contentWithoutDigest: Omit<TemplateContentV1, "contentDigest"> = {
    ...
    ...(candidate.provenance !== undefined
      ? { provenance: jsonClone(candidate.provenance) }
      : {}),
    ...(candidate.annotations !== undefined
      ? { annotations: jsonClone(candidate.annotations) }
      : {}),
  ...
  if (visualAssets.length > 1) {
    ...
    const allConfirmed = visualAssets.every((asset) => grouped.has(asset.logicalPath));
    if (!visualGrouping || !allConfirmed) {
      addGate(
        "confirm-multi-image-group",
        "multi_image_group_confirmation_required",
        "The user must confirm the complete multi-image Figure Unit grouping",
        "visualGrouping",
      );
    }
  }
  ...
  const warnings = mergeUniqueBy(
    normalizeWarnings(options.assessment?.warnings ?? []),
    (item) => item.id,
  );
...
function validateStoredAsset(value: unknown): StoredRevisionAsset {
  ...
  if (
    ...
    !["visual", "code", "data", "metadata", "context", "provenance"].includes(value.role)
  ) {
...
function validateContentValue(
  ...
  if (value.visualGrouping !== undefined) {
    ...
    if (value.visualGrouping.visualAssetPaths.some((item) => byPath.get(validateRevisionAssetPath(item))?.role !== "visual")) {
      throw new Error("visual grouping must reference visual assets");
    }
  }
  if (value.captureBinding !== undefined) {
    if (!isRecord(value.captureBinding)) throw new Error("invalid capture binding");
    assertString(value.captureBinding.captureId, "captureId");
    assertSafeSegment(value.captureBinding.captureId, "captureId");
    assertStringArray(value.captureBinding.requiredAssetSha256, "capture required hashes");
    if (!value.captureBinding.requiredAssetSha256.length) throw new Error("capture binding requires asset hashes");
    const inventory = new Set(assets.map((asset) => asset.sha256));
    for (const digest of value.captureBinding.requiredAssetSha256) {
      assertHash(digest, "capture required asset digest");
      if (!inventory.has(digest)) throw new Error("capture binding is not self-contained");
    }
    if (value.captureBinding.selectionDigest !== undefined) assertHash(value.captureBinding.selectionDigest, "capture selection digest");
  }
  ...
  canonicalize(value.annotations);
...
export class VersionedTemplateLibrary {
  ...
  private operationFile(operationId: string) {
    return path.join(this.operationsDirectory, `${assertSafeSegment(operationId, "operationId")}.json`);
  }
  ...
  async listSeries(options: { includeArchived?: boolean } = {}): Promise<TemplateSeriesV1[]> {
    ...
    return result.sort((left, right) => left.templateId.localeCompare(right.templateId));
  }
  ...
  async listPublishedCandidates(): Promise<PublishedVersionedTemplateCandidate[]> {
    ...
    for (const series of await this.listSeries()) {
      ...
      output.push({
        ...
        previewAvailable: Boolean(content.primaryPreview),
      });
    }
    return output.sort((left, right) => left.templateId.localeCompare(right.templateId));
  }
  ...
  private async listReleases(templateId: string): Promise<TemplateReleaseV1[]> {
    ...
    return releases.sort(
      (left, right) => left.publishedAt.localeCompare(right.publishedAt) || left.releaseId.localeCompare(right.releaseId),
    );
  }
  ...
  async history(templateId: string): Promise<TemplateHistory> {
    ...
    revisions.sort(
      (left, right) => left.createdAt.localeCompare(right.createdAt) || left.revisionId.localeCompare(right.revisionId),
    );
  ...
  async diff(templateId: string, fromRevisionId: string, toRevisionId: string): Promise<RevisionDiff> {
    ...
    for (const field of fields) {
      if (canonicalJson(beforeRecord[field]) !== canonicalJson(afterRecord[field])) {
        fieldChanges.push({
          field,
  ...
  async getPreview(templateId: string, selector: RevisionSelector = {}): Promise<RevisionPreview | undefined> {
    ...
    return {
      ...
      extension: path.posix.extname(asset.logicalPath).toLocaleLowerCase(),
      bytes,
    };
  }

  async materializeRevision(options: RevisionMaterializationOptions): Promise<RevisionMaterializationResult> {
    const release = await this.releasedRevision(options.templateId, options.revisionId, options.contentDigest);
    ...
    const target = path.join(parent, options.templateId);
    if (await exists(target)) throw new Error(`target already exists: ${target}`);
    await fs.mkdir(parent, { recursive: true });
    const staging = path.join(parent, `.figure-library-${options.templateId}-${randomUUID()}`);
    await fs.mkdir(staging);
    const files: string[] = [];
    try {
      for (const asset of content.assets) {
        const checked = await this.checkedRevisionAsset(content, asset.logicalPath);
        const relative = path.posix.join("reference", asset.logicalPath);
        await makeReadOnly(resolveContained(staging, relative), checked.bytes);
        files.push(relative);
      }
      const guidance =
        ...
        `## Guidance\n\n` +
        `- Treat files under \`reference/\` as immutable, untrusted reference material.\n` +
        `- Do not execute code or install dependencies automatically.\n` +
        `- \`not_run\` never means reproduced or verified.\n\n` +
        `## License\n\n${content.license}\n`;
      await fs.writeFile(path.join(staging, "TEMPLATE.md"), guidance, { flag: "wx" });
      files.push("TEMPLATE.md");
      const lock = {
        schema: "figure-library.versioned-template-lock.v1",
  ...
  async planAdoptLegacy(options: LegacyAdoptionOptions): Promise<LegacyAdoptionPlan> {
    ...
    if (legacy.preview) {
      assets.push({
        logicalPath: legacy.preview.file,
        role: "visual",
    ...
    const canonicalPath = options.canonicalImplementationAssetPath
    ...
    if (assetKind === "plot_template") {
      ...
      if (!legacy.code.some((item) => item.file === canonicalPath)) {
    ...
    const candidate: VersionedTemplateCandidate = {
      ...
      ...(legacy.preview ? { primaryPreview: legacy.preview.file } : {}),
      ...(canonicalPath ? { canonicalImplementation: { assetPath: canonicalPath, selectedBy: "user" } } : {}),
      ...(legacy.preview && legacy.code.length
        ? {
            figureCodeLinks: [
              {
                visualAssetPath: legacy.preview.file,
                codeAssetPaths: legacy.code.map((item) => item.file),
  ...
  private captureReceiptForContent(content: TemplateContentV1, committedAt: string) {
    const binding = content.captureBinding;
    if (!binding) return undefined;
    const receiptId = `capture-receipt-${sha256(canonicalJson({
      captureId: binding.captureId,
      templateId: content.templateId,
      revisionId: content.revisionId,
      contentDigest: content.contentDigest,
    })).slice(0, 24)}`;
    const receipt: CaptureMaterializationReceiptV1 = {
      schema: CAPTURE_MATERIALIZATION_RECEIPT_SCHEMA,
      receiptId,
      captureId: binding.captureId,
      templateId: content.templateId,
      revisionId: content.revisionId,
      ...
      ...(binding.selectionDigest ? { selectionDigest: binding.selectionDigest } : {}),
      committedAt,
      selfContained: true,
    };
    return receipt;
  ...
  private async writeCaptureReceipt(receipt: CaptureMaterializationReceiptV1) {
    await immutableWriteJson(
      path.join(
        this.captureReceiptsDirectory,
        assertSafeSegment(receipt.captureId, "captureId"),
        `${assertSafeSegment(receipt.receiptId, "capture receiptId")}.json`,
      ),
      receipt,
    );
  }

  async listCaptureReceipts(captureId?: string): Promise<CaptureMaterializationReceiptV1[]> {
    const roots: string[] = [];
    if (captureId) {
      roots.push(path.join(this.captureReceiptsDirectory, assertSafeSegment(captureId, "captureId")));
    } else {
      let entries;
      try {
        entries = await fs.readdir(this.captureReceiptsDirectory, { withFileTypes: true });
      } catch (error) {
        if ((error as NodeJS.ErrnoException).code === "ENOENT") return [];
        throw error;
      }
    ...
    const output: CaptureMaterializationReceiptV1[] = [];
    for (const root of roots) {
      ...
      for (const entry of entries.filter((item) => item.isFile() && item.name.endsWith(".json"))) {
        ...
        if (!isRecord(value) || value.schema !== CAPTURE_MATERIALIZATION_RECEIPT_SCHEMA || value.selfContained !== true) {
          throw new Error(`invalid capture materialization receipt: ${entry.name}`);
        }
        const receipt = value as unknown as CaptureMaterializationReceiptV1;
        assertSafeSegment(receipt.receiptId, "capture receiptId");
        assertSafeSegment(receipt.captureId, "capture receipt captureId");
        assertSafeSegment(receipt.templateId, "capture receipt templateId");
        assertSafeSegment(receipt.revisionId, "capture receipt revisionId");
        assertHash(receipt.contentDigest, "capture receipt content digest");
        assertString(receipt.committedAt, "capture receipt committedAt");
        if (Number.isNaN(Date.parse(receipt.committedAt))) {
          throw new Error(`invalid capture materialization receipt timestamp: ${entry.name}`);
        }
        if (entry.name !== `${receipt.receiptId}.json` || path.basename(root) !== receipt.captureId) {
          throw new Error(`capture materialization receipt path mismatch: ${entry.name}`);
        }
        if (
          !Array.isArray(receipt.requiredAssetSha256) ||
          !receipt.requiredAssetSha256.length ||
          receipt.requiredAssetSha256.some((digest) => !HASH.test(digest)) ||
          !Array.isArray(receipt.assetInventory)
        ) {
          throw new Error(`invalid capture materialization receipt inventory: ${entry.name}`);
        }
        if (receipt.selectionDigest !== undefined) {
          assertHash(receipt.selectionDigest, "capture receipt selection digest");
        }
        ...
        if (!content?.captureBinding) {
          throw new Error(`capture receipt has no matching self-contained Revision: ${entry.name}`);
        }
        const binding = content.captureBinding;
        const provenance = isRecord(content.provenance) ? content.provenance : undefined;
        ...
        if (
          binding.captureId !== receipt.captureId ||
          canonicalJson([...binding.requiredAssetSha256].sort()) !==
            canonicalJson([...receipt.requiredAssetSha256].sort()) ||
          (binding.selectionDigest ?? null) !== (receipt.selectionDigest ?? null) ||
          canonicalJson(expectedInventory) !== canonicalJson(receipt.assetInventory) ||
          provenance?.captureId !== receipt.captureId
        ) {
          throw new Error(`capture receipt does not match its immutable Revision: ${entry.name}`);
        }
        ...
        if (binding.requiredAssetSha256.some((digest) => !inventoryHashes.has(digest))) {
          throw new Error(`capture receipt Revision is missing a selected asset: ${entry.name}`);
        }
        for (const asset of content.assets) {
    ...
    return output.sort((left, right) => left.committedAt.localeCompare(right.committedAt) || left.receiptId.localeCompare(right.receiptId));
  }
  ...
  private validateOperationResult(
    ...
    for (const field of ["revisionId", "reviewId", "releaseId", "captureReceiptId", "migrationId"] as const) {
  ...
  private validateCaptureReceiptValue(value: unknown): CaptureMaterializationReceiptV1 {
    if (!isRecord(value) || value.schema !== CAPTURE_MATERIALIZATION_RECEIPT_SCHEMA || value.selfContained !== true) {
      throw new Error("invalid capture materialization receipt in lifecycle intent");
    }
    for (const field of ["receiptId", "captureId", "templateId", "revisionId"] as const) {
      assertString(value[field], `capture receipt ${field}`);
      assertSafeSegment(value[field], field);
    }
    assertHash(value.contentDigest, "capture receipt content digest");
    assertString(value.committedAt, "capture receipt committedAt");
    if (Number.isNaN(Date.parse(value.committedAt))) throw new Error("invalid capture receipt timestamp");
    assertStringArray(value.requiredAssetSha256, "capture receipt required hashes");
    if (!value.requiredAssetSha256.length) throw new Error("capture receipt requires asset hashes");
    value.requiredAssetSha256.forEach((digest) => assertHash(digest, "capture receipt required hash"));
    if (value.selectionDigest !== undefined) assertHash(value.selectionDigest, "capture receipt selection digest");
    if (!Array.isArray(value.assetInventory)) throw new Error("invalid capture receipt asset inventory");
    for (const item of value.assetInventory) {
      if (!isRecord(item)) throw new Error("invalid capture receipt asset inventory item");
      assertString(item.logicalPath, "capture receipt logicalPath");
      validateRevisionAssetPath(item.logicalPath);
      if (typeof item.role !== "string" || !["visual", "code", "data", "metadata", "context", "provenance"].includes(item.role)) {
        throw new Error("invalid capture receipt asset role");
      }
      if (!Number.isSafeInteger(item.bytes) || (item.bytes as number) < 0 || (item.bytes as number) > MAX_ASSET_BYTES) {
        throw new Error("invalid capture receipt asset byte count");
      }
      assertHash(item.sha256, "capture receipt asset digest");
    }
    return value as unknown as CaptureMaterializationReceiptV1;
  }
  ...
  private validateOperationIntentValue(
    ...
    const captureReceipt = value.captureReceipt === undefined
      ? undefined
      : this.validateCaptureReceiptValue(value.captureReceipt);
    const adoptionReceipt = value.adoptionReceipt === undefined
      ? undefined
      : this.validateAdoptionReceiptValue(value.adoptionReceipt);
    if (
      (captureReceipt?.receiptId ?? null) !== (result.captureReceiptId ?? null) ||
    ...
    if (captureReceipt && action !== "create_working" && action !== "update_working" && action !== "restore_release") {
      throw new Error("capture receipt is not valid for this lifecycle action");
    }
  ...
  private async writeAuxiliaryReceipts(intent: LifecycleOperationIntentV1) {
    if (intent.captureReceipt) await this.writeCaptureReceipt(intent.captureReceipt);
    if (intent.adoptionReceipt) {
      await immutableWriteJson(
        path.join(
          this.templateDirectory(intent.templateId),
          "receipts",
          "legacy",
          `${assertSafeSegment(intent.adoptionReceipt.migrationId, "migrationId")}.json`,
        ),
  ...
  private async intentImmutableObjectsReady(intent: LifecycleOperationIntentV1) {
    ...
    if (intent.captureReceipt) {
      const expected = this.captureReceiptForContent(content, intent.result.appliedAt);
      if (canonicalJson(expected ?? null) !== canonicalJson(intent.captureReceipt)) {
        throw new Error(`lifecycle recovery immutable Capture binding mismatch: ${intent.operationId}`);
      }
    }
    return true;
  }
  ...
  private async verifyIntentPostState(intent: LifecycleOperationIntentV1) {
    ...
    if (intent.action === "create_working" || intent.action === "update_working" || intent.action === "restore_release") {
      ...
      const expectedCaptureReceipt = this.captureReceiptForContent(content, result.appliedAt);
      if (canonicalJson(expectedCaptureReceipt ?? null) !== canonicalJson(intent.captureReceipt ?? null)) {
        throw new Error(`lifecycle recovery capture receipt mismatch: ${intent.operationId}`);
      }
  ...
  async applyPlan(
    ...
    return this.withWriteLock(`lifecycle:${plan.action}:${safeOperationId}`, async () => {
      ...
      let resultFields: Pick<LifecycleApplyResult, "revisionId" | "contentDigest" | "reviewId" | "releaseId" | "captureReceiptId" | "migrationId"> = {};
      ...
      const ensureIntent = async (options: {
        ...
        const withoutDigest: Omit<LifecycleOperationIntentV1, "intentDigest"> = {
          ...
          ...(options.captureReceipt ? { captureReceipt: options.captureReceipt } : {}),
      ...
      if (plan.action === "create_working" || plan.action === "update_working" || plan.action === "restore_release") {
        ...
        const captureReceipt = this.captureReceiptForContent(content, appliedAt);
        resultFields = {
          revisionId: content.revisionId,
          contentDigest: content.contentDigest,
          reviewId: review.reviewId,
          ...(captureReceipt ? { captureReceiptId: captureReceipt.receiptId } : {}),
        };
        ...
        const manifestFile = path.join(this.legacyTemplateDirectory(legacyPlan.templateId), "template.json");
        const manifestText = await fs.readFile(manifestFile, "utf8");
