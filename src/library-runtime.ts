...
export const LIBRARY_ROOT_MARKER_SCHEMA = "figure-library.root-marker.v1" as const;
export const LIBRARY_LOCATOR_SCHEMA = "figure-library.locator.v1" as const;
...
export const LEGACY_LIBRARY_COPY_RECEIPT_SCHEMA =
  "figure-library.legacy-library-copy-receipt.v1" as const;
export const LIBRARY_ROOT_MARKER_FILE = ".figure-library-root.json" as const;
...
function canonicalize(value: unknown): unknown {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) throw new Error("library metadata contains a non-finite number");
    return value;
  }
  if (Array.isArray(value)) return value.map(canonicalize);
  if (typeof value !== "object") throw new Error("library metadata is not JSON serializable");
...
export async function ensureLibraryRootMarker(root: string, expectedLibraryId?: string) {
...
function excludedTargetBindingState(relativePath: string) {
  return (
    ...
    relativePath === ".write-lock" ||
    relativePath.startsWith(".write-lock/") ||
    relativePath === "store/receipts/library-bindings" ||
    relativePath.startsWith("store/receipts/library-bindings/")
  );
}
...
export async function planGlobalLibraryBinding(options: {
  ...
  if (migrationMode === "copy_legacy") {
    ...
    await assertMigrationTargetConflicts(libraryDirectory, sourceInventory);
    migration = {
      mode: "copy_legacy",
      sourceDirectory,
      sourceInventory,
  ...
  const validateInventory = (inventory: LibraryFileInventoryEntry[], label: string) => {
    ...
    for (const entry of inventory) {
      if (
        ...
        (previous && previous.localeCompare(entry.relativePath) >= 0)
      ) {
        ...
        async () => {
          ...
          if (plan.migration.mode === "copy_legacy") {
            ...
            if (
              ...
              plan.migration.sourceInventory.some((source) => {
            ...
            migrationReceiptFile = path.join(
              ...
              "receipts",
              "library-bindings",
              `${assertPortableSegment(plan.bindingId, "bindingId")}.json`,
            );
            ...
            if (!migrationReceipt) {
              const receipt: LegacyLibraryCopyReceiptV1 = {
                ...
                sourceDirectory: plan.migration.sourceDirectory,
                targetDirectory: plan.libraryDirectory,
