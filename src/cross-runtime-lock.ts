...
function canonicalize(value: unknown): unknown {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) throw new Error("lock metadata contains a non-finite number");
    return value;
  }
  if (Array.isArray(value)) return value.map(canonicalize);
  if (typeof value !== "object") throw new Error("lock metadata is not JSON serializable");
...
export class CrossRuntimeWriteLock {
  ...
  constructor(options: CrossRuntimeWriteLockOptions) {
    ...
    this.lockDirectory = path.resolve(
      options.lockDirectory ?? path.join(this.root, ".write-lock"),
    );
...
export async function planLibraryWriteLockRecovery(options: {
  ...
  const lockDirectory = path.resolve(
    options.lockDirectory ?? path.join(libraryRoot, ".write-lock"),
  );
...
function validateRecoveryPlan(plan: LibraryWriteLockRecoveryPlanV1) {
  ...
  if (path.resolve(plan.lockDirectory) !== path.join(path.resolve(plan.libraryRoot), ".write-lock")) {
...
export async function applyLibraryWriteLockRecovery(
  ...
  const recoveryRoot = path.join(plan.libraryRoot, ".write-lock-recovery");
  const receiptFile = path.join(recoveryRoot, "receipts", `${safeOperationId}.json`);
  const archiveDirectory = path.join(
    recoveryRoot,
    "archives",
    `${safeOperationId}-${plan.expectedLockDigest.slice(0, 16)}`,
  );
  ...
  const guard = new CrossRuntimeWriteLock({
    root: recoveryRoot,
    lockDirectory: path.join(recoveryRoot, ".apply-lock"),
...
export async function hasLibraryWriteLock(root: string) {
  return exists(path.join(path.resolve(root), ".write-lock"));
}
